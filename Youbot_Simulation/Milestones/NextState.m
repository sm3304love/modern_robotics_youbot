...
function [updated_configuration] = NextState(current_configuration, control_velocity, time_step, control_velocity_limits)
...
Pseudo_H0 = [-0.0308,0.0308,0.0308,-0.0308; ones(1,4)*0.0119;-0.0119,0.0119,-0.0119,0.0119];

% delta_qb
% Pseudo_H0*wheel_velocity(:)*time_step; -> V_b*delta_t -> speed*time 
% speed*time -> delta_qb 

chassis_state(:) = chassis_state(:) + [1,0,0;
                                       0,cos(phi),-sin(phi);0,sin(phi),cos(phi)]*Pseudo_H0*wheel_velocity(:)*time_step;
